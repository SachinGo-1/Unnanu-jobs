...
class JobResults extends Component {
  ...
  loadItems(page) {
    ...
    if (!this.state.pageLoading && this.state.hasMoreItems) {
      ...
      this.setState({ pageLoading: true }, () => {
        ...
        let filtering = `${locationFiltering +
          closing +
          jobFilter +
          workFilter +
          salaryFilter +
          keywordFiltering} and (status eq 2 or status eq null) and closing_date gt ${formatDate}`;
        ...
        facade
          .get(
            `/indexes/job-board-uat/docs?api-version=2017-11-11&search=*&$skip=${pageNumber}&$top=10&$filter=${filtering}&$count=true&facet=closing_date,count:10000&facet=job_type_array_facet&facet=job_schd_facet&facet=salary_annually_facet,count:10000${sort}`
          )
          .then((resp) => {
  ...
  render() {
    ...
    jobList.map((item, key) => {
      ...
      items.push(
        <JobsCard
          ...
          salary={formatSalary(item.salary_hr_start, item.salary_hr_end, item.salary_text_type)}
    ...
    return (
      ...
      >
        {!noResults && !pageLoading && !initialLoad && (
