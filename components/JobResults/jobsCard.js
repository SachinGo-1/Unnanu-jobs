...
import {imageURL, isValidDate, prepareURLName} from "services/utils";

// import ApplyButton from "components/Buttons/ApplyButton";

class JobsCard extends Component<PropTypes> {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        if (jobWorkType.length === 2) {
            ...
            [, jobLocationType] = jobWorkType[0].split(" - ");
        }
        const loctag = {
            marginLeft: "13px",
            padding: "0 13px",
            ...
            <div className="card-text-content">
              <div className="result-card-header">
                <div className="job-title">
                  <a
                    href={`/job/${id}/${jobNameURL}`}
                    onClick={(e) => showJobPopup(e, id, jobNameURL)}
                  >
                    {title}
                  </a>
                ...
                <div className="job-company-location">
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="14" viewBox="0 0 10 14"><g fill="#000" fill-rule="nonzero" opacity=".2"><path d="M5 0C2.514 0 .5 2.015.5 4.5c0 3.15 4.063 8.843 4.235 9.085a.33.33 0 0 0 .53 0C5.437 13.343 9.5 7.65 9.5 4.5 9.5 2.015 7.486 0 5 0zm0 6.75A2.25 2.25 0 1 1 5 2.25a2.25 2.25 0 0 1 0 4.5z"/></g></svg>

                  <div>{`${company} · ${data.location}`}</div>
                </div>
                <div className="job-salary-and-close-date">
              ...
              <div className="result-card-ui-actions">
                ...
                {isValidDate(closeDate) &&
                  <div className="timestamp" style={{color: "#222"}}>
                              Close: <Moment format="MMM Do YYYY">{closeDate}</Moment>
                  </div>
                }
              ...
              <div className="job-type-badges">
                <button className="badge-full-time">Full Time</button>
                <button className="badge-urgent">Urgent</button>
              </div>
              <SubmitButton
                submitting={false}
                text={applyBtnText}
              ...
              <div className="timestamp-mobile">
                        Closes on <Moment format="MMM Do, YYYY">{closeDate}</Moment>
              </div>
            </div>
          </div>
        );
...
JobsCard.propTypes = {
    ...
    jobWorkType: PropTypes.arrayOf(PropTypes.string).isRequired,
...
export default withRedux(Store, mapStateToProps)(withReduxSaga(JobsCard));
