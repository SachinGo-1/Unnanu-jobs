...
class SubmitButton extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { submitting, resetAfterComplete, buttonMessageTimeout } = this.props;
  ...
  render() {
    ...
    if (status === "working") {
      ...
      buttonText = <span>Success!</span>;
    } else if (status === "fail") {
      buttonText = <span>{errorText}</span>;
    } else {
    ...
    const btnCss = classNames({
      ...
      "apply-button":
        btnState === "apply" ||
        btnState === "applied" ||
        btnState === "mobile-apply" ||
        btnState === "mobile-applied",
      "apply-button-mobile": btnState === "mobile-apply" || btnState === "mobile-applied",
      "applied": btnState === "applied" || btnState === "mobile-applied",
      "save-button": btnState === "save" || btnState === "saved",
      "saved": btnState === "saved",
      "large": size === "large",
      "in-progress": btnState === "in-progress",
      "withdraw": btnState === "withdraw",
      "delete": btnState === "delete",
    });

    let btnContents = buttonText;

    if (btnState === "applied" || btnState === "mobile-applied") {
      btnContents = (
        <span>
    ...
    if (btnState === "delete") {
      btnContents = (
        <span>
          ...
          >
            <path d="M2.5 5h15m-1.667 0v11.667c0 .833-.833 1.667-1.667 1.667H5.833c-.833 0-1.667-.833-1.667-1.667V5m2.5 0V3.333c0-.833.833-1.667 1.667-1.667h3.333c.833 0 1.667.833 1.667 1.667V5m-5 4.167v5m3.333-5v5" />
          </svg>
        </span>
      );
    }
