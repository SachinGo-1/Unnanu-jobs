...
import { prepareURLName } from "services/utils";
...
class Modal extends React.Component {
  ...
  onKeyPress(e) {
    if (e.keyCode === 37) {
      const getData =
        this.props.type === "Job"
          ? this.compilePreviousAndNext(this.props.id)
  ...
  compilePreviousAndNext(id) {
    ...
    try {
      ...
      if (getIndex === 0 && getIndex === jobs.length - 1) {
        ...
        leftJobNameURL = prepareURLName({
          title: jobs[getIndex - 1].j_title || jobs[getIndex - 1].title,
          company:
            jobs[getIndex - 1].company_name || jobs[getIndex - 1].company,
        });
        rightJobNameURL = prepareURLName({
          title: jobs[getIndex + 1].j_title || jobs[getIndex + 1].title,
          company:
            jobs[getIndex + 1].company_name || jobs[getIndex + 1].company,
        });
      ...
      return {
        left: previous,
        right: next,
        leftName: leftJobNameURL,
        rightName: rightJobNameURL,
      };
    } catch (e) {
      return {
        left: previous,
    ...
    if (this.props.type === "Job") {
      content = (
        <JobPage
          ...
          isSimilarJobs={this.props.similarJobs}
      ...
      content = (
        <Signup
          id={this.props.id}
          applyJobID={this.props.jobID}
          actionType={this.props.actionType}
        />
      );
    } else if (this.props.type === "SavedJobs") {
      content = (
        <SavedJobs data={this.props.data} close={this.props.onDismiss} />
      );
    } else if (this.props.type === "AppliedJobs") {
      content = (
        <AppliedJobs data={this.props.data} close={this.props.onDismiss} />
      );
    } else {
      content = null;
    }
        ...
        <div ref={(el) => (this._photoWrap = el)} className="modal-main">
          {showLeftButton && (
            <button
              ...
              onClick={(e) =>
                this.previousJob(e, getData.left, getData.leftName)
              }
            >
              <svg width="22px" height="40px" viewBox="0 0 22 40" version="1.1">
                ...
                >
                  <g
                    id="un-jobpage-job-board---job-post/popularjobs"
                    transform="translate(-224.000000, -404.000000)"
                    fill="#BDBDBD"
                    stroke="#BDBDBD"
                    strokeWidth="0.8"
                  >
          ...
          {showRightButton && (
            <button
              ...
              onClick={(e) => this.nextJob(e, getData.right, getData.rightName)}
            >
              <svg width="22px" height="40px" viewBox="0 0 22 40" version="1.1">
                <g
                  id="JobB"
                ...
                >
                  <g
                    id="un-jobpage-job-board---job-post/popularjobs"
                    transform="translate(-1194.000000, -404.000000)"
                    fill="#BDBDBD"
                    stroke="#BDBDBD"
                    strokeWidth="0.8"
                  >
...
export default withRedux(Store, mapStateToProps)(withReduxSaga(Modal));
