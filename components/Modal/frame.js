...
import {
  ...
  imageURL,
  prepareURLQuery,
  prepareURLName, formatSalary, isValidDate,
} from "services/utils";
...
class JobPage extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      Object.keys(oldPostId).length === 0 &&
      oldPostId.constructor === Object &&
      newPostId
    ) {
      checkCompanyJobs({
        company: newPostId.company_name,
        location: newPostId.location,
        id: newPostId.j_id,
      });
    }
  }
  ...
  applyJob(id) {
    ...
    if (!appData.isLogged && appData.isModalOpen) {
      ...
      window.location = `${
        urls.Recruit
      }/jobboard/apply/${formatJobId}?source=UJSL`;
    }
  }
  ...
  render() {
    ...
    if (isSaved || isPopular) {
      ...
      const sideListJobs = isSaved
        ? this.getOtherSavedJobs(modalJob.j_id)
        : this.getOtherPopularJobs(modalJob.j_id);
      jobListSide = (
        <JobsList
          heading={`Other ${title_jobs} jobs`}
          jobs={sideListJobs}
          isPopup
        />
      );
        ...
        {showJob && (
          <div className="popup-wrapper">
            <div className="popup-content">
              <div className="popup-header">
                ...
                <div className="header-content">
                  ...
                  <div className="job-company-location">
                    ...
                    {`${modalJob.company_name} · ${modalJob.location}`}
                  ...
                  {formatSalary(modalJob.salary_hr_start, modalJob.salary_hr_end, modalJob.salary_text_type) !== "Salary Not Specified" && (
                    <div className="job-salary-range">
                      ...
                      {/* {this.composeSalaryRange(modalJob.salary_type, modalJob)} */}
              ...
              <div className="popup-body">{renderHTML(modalJob.text)}</div>
              <hr/>
              <div className="job-post-footer d-flex justify-content-between">
                {isValidDate(modalJob.updated_time) && (
              ...
              {isApplied && !isJobExpired ?
                ...
                />) : (
                  <ApplyButton
                    ...
                    size="large"
                    handleClick={
                      !isApplied ? () => this.applyJob(modalJob.j_id) : null
                    }
