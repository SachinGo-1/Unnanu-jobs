...
import { SocketListner } from "services/api/socket";

import { fetchNotifications, signOut } from "store/app/actions";
import { clearToken } from "store/localStorage";

import { resetSorting } from "store/posts/actions";

import { prepareURLQuery, profileImageURL } from "services/utils";
...
class LoggedIn extends Component {
  ...
  componentDidMount() {
    if (this.refs.video && this.refs.video2) {
      this.refs.video.play();
      this.refs.video2.play();
    }
    const cookies = parseCookies();
    this.socketHandler(this.props.appData.token || cookies.token);
  ...
  getMessageCount = (array) => {
    ...
    array.forEach(function(message) {
      count = count + message.Unread_Message_Count;
    });
    return count;
  };

  socketHandler = (token) => {
    SocketListner(token, (data) => {
      this.getRecount();
    });
  };
  ...
  getRecount = () => {
    ...
    const cookies = parseCookies();
    fetchNotifications(cookies.token);
  };
  ...
  render() {
    ...
    const isUserLogged = userData && userData.isLogged && userData.user;

    const isDPVideo =
      userData.isLogged && userData.user
        ? userData.user.BasicInformation.isDPVideo
        : 0;
    ...
    if (isDPVideo) {
      profilePicture = (
        <video
          ...
          width="31"
          height="31"
          src={profileImageURL(userData.user.BasicInformation.DpUrl)}
          style={profileCSS}
          muted
        />
      );
      profilePictureDropdown = (
        <video
          ...
          autoPlay
          loop
          className="user-account-menu-profile"
          width="42"
          height="42"
          src={profileImageURL(userData.user.BasicInformation.DpUrl)}
          style={profileCSS}
      ...
      profilePicture = userData.user.BasicInformation.DpUrl ? (
        <img
          src={profileImageURL(userData.user.BasicInformation.DpUrl)}
          alt="profile pic"
        />
        ...
        >
          <defs>
            <linearGradient id="b" x1="50%" x2="50%" y1="0%" y2="108.151%">
          ...
          <g fill="none" fillRule="evenodd">
            ...
            <g fillRule="nonzero">
              ...
              <use fill="url(#b)" fillOpacity=".4" xlinkHref="#a" />
            </g>
            <g fill="#EEE" fillRule="nonzero" mask="url(#c)">
              <g transform="translate(15.09 36.218)">
                <circle cx="67.909" cy="25.655" r="25.655" />
                <path d="M69.418 63.382C31.08 63.382 0 94.462 0 132.8h138.836c0-38.339-31.08-69.418-69.418-69.418z" />
              </g>
            </g>
          </g>
        </svg>
      ...
      profilePictureDropdown = userData.user.BasicInformation.DpUrl ? (
        <img
          className="user-account-menu-profile"
          src={profileImageURL(userData.user.BasicInformation.DpUrl)}
          alt="profile pic"
    ...
    const isNotificationEmpty =
      ...
      notifications.constructor === Object;
    if (!isNotificationEmpty) {
      messagesCount = this.getMessageCount(notifications.UnreadMessges);
      offersCount = notifications.UnreadOffers.length;
      interviewsCount = notifications.UnreadInterviews.length;
    }

    return (
      <div
        className="logged-in-user-navigation container-fluid"
        ref="headerRef"
      >
            ...
            >
              ...
              >
                ...
                >
                  <g
                    id="unnanu"
                    transform="translate(-8.000000, -15.000000)"
                    fill="#FFFFFF"
                  >
                    <g
                      id="unnanu-logo"
          ...
          <div className="collapse navbar-collapse">
            <ul className="navbar-nav user-navbar-nav mx-auto">
              ...
              <li className="nav-item nav-item-messages">
                <a className="nav-link" href={`${urls.Recruit}/message`}>
                  <svg width="17" height="16" viewBox="0 0 17 16">
                    <g fill="#FFF" fillRule="nonzero">
                      <path d="M17 0h-5.667v12H17V0zm-5.667 0H0v12h2.267v4l5.288-4h3.778V0zm4.534 10.857h-8.5L3.4 13.714v-2.857H1.133V1.143h14.734v9.714z" />
                      <path d="M3.4 5.143h5.667a.569.569 0 0 0 .566-.572A.569.569 0 0 0 9.067 4H3.4a.569.569 0 0 0-.567.571c0 .316.254.572.567.572zM2.833 7.429c0 .316.196.571.436.571h10.462c.24 0 .436-.255.436-.571 0-.316-.196-.572-.436-.572H3.269c-.24 0-.436.256-.436.572z" />
                    </g>
                  </svg>
                  Messages
                </a>
                {messagesCount > 0 && (
                  <span className="notifications">{messagesCount}</span>
                )}
              </li>
              <li className="nav-item nav-item-interviews">
                <a className="nav-link" href={`${urls.Recruit}/interviews`}>
                  <svg width="17" height="16" viewBox="0 0 17 16">
                    <g fill="#FFF" fillRule="nonzero">
                      <path d="M8.35 8.968a4.486 4.486 0 0 0 4.485-4.485A4.474 4.474 0 0 0 8.351.013c-2.477 0-4.485 2.008-4.485 4.47A4.494 4.494 0 0 0 8.35 8.968zm0-7.786a3.311 3.311 0 0 1 3.302 3.301 3.311 3.311 0 0 1-3.301 3.302 3.311 3.311 0 0 1-3.302-3.302A3.311 3.311 0 0 1 8.35 1.182zM.592 15.97h15.573c.33 0 .591-.26.591-.59a5.633 5.633 0 0 0-5.626-5.628H5.627A5.633 5.633 0 0 0 0 15.38c0 .33.261.592.592.592zm5.035-5.034h5.503a4.44 4.44 0 0 1 4.402 3.852H1.224a4.452 4.452 0 0 1 4.403-3.852z" />
                    </g>
                  </svg>
                  Interviews
                </a>
                {interviewsCount > 0 && (
                  <span className="notifications">{interviewsCount}</span>
                )}
              </li>
              <li className="nav-item nav-item-offers">
                <a className="nav-link" href={`${urls.Recruit}/offers`}>
                  <svg width="19" height="16" viewBox="0 0 19 16">
                    <path
                      fill="#FFF"
                      fillRule="nonzero"
                      d="M18.369 2.803a1.33 1.33 0 0 0-.986-.413h-4.05V.66a.631.631 0 0 0-.63-.642H6.067a.64.64 0 0 0-.642.642v1.73H1.387c-.39 0-.72.14-.986.413-.26.266-.401.596-.401.974v10.258c0 .379.134.714.401.986.26.267.596.401.986.401h15.996c.39 0 .725-.134.986-.4a1.37 1.37 0 0 0 .4-.987V3.777c0-.378-.14-.708-.4-.974zM6.697 1.29h5.375v1.1H6.697v-1.1zm10.8 12.745c0 .08-.034.115-.114.115H1.387c-.058 0-.126-.069-.126-.115V8.281h6.49v.31c0 .355.276.63.631.63h1.994a.631.631 0 0 0 .642-.63v-.31h6.48v5.754zM1.262 7.02V3.777c0-.08.045-.115.126-.115h15.996c.08 0 .115.035.115.115V7.02H1.26z"
                    />
                  </svg>
                  Offers
                </a>
                {offersCount > 0 && (
                  <span className="notifications">{offersCount}</span>
                )}
              </li>
              <li
                className={`nav-item nav-item-find-jobs ${urlDetails !==
                  undefined &&
                  urlDetails.pathname === "/" &&
                  "active"}`}
              >
                <a className="nav-link" href="/">
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    stroke={`${
                      urlDetails !== undefined && urlDetails.pathname === "/"
                        ? "#ffc107"
                        : "#000"
                    }`}
                  >
                  ...
                  Jobs for{" "}
                  {userData.user.BasicInformation.FullName.split(" ")[0]}
                </a>
              </li>
              {/*<li className={`nav-item nav-item-find-jobs ${urlDetails.pathname === '/match' ? 'active' : ''}`}>*/}
              {/*  <a className="nav-link" href={`${urls.BaseURL}/match`}>*/}
              {/*    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="15" viewBox="0 0 24 24"*/}
              {/*         stroke={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`}*/}
              {/*    >*/}
              {/*      <path fill={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`} fillRule="nonzero"*/}
              {/*            d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/>*/}
              {/*      <path fill={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`} fillRule="nonzero"*/}
              {/*            d="M20 3v4"/>*/}
              {/*      <path fill={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`} fillRule="nonzero"*/}
              {/*            d="M22 5h-4"/>*/}
              {/*      <path fill={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`} fillRule="nonzero" d="M4 17v2" />*/}
              {/*      <path fill={`${urlDetails.pathname === '/match' ? "#266ADC" : "#FFFFFF"}`} fillRule="nonzero" d="M5 18H3" />*/}
              {/*    </svg>*/}
              {/*    Jobs for you*/}
              {/*  </a>*/}
              {/*</li>*/}
            </ul>
            <div className="navbar-nav user-menu-right d-flex my-2 my-lg-0">
              <div
                className="user-account-menu-name"
              ...
              <div className="user-profile ml-auto btn-group">
                <button
                  className="profile-button btn dropdown-toggle"
                  type="button"
                  id="dropdownMenu2"
                ...
                <div
                  className="user-account-menu dropdown-menu dropdown-menu-right"
                  aria-labelledby="dropdownMenu2"
                >
                  <div className="user-account-menu-header">
                    {profilePictureDropdown}
                    <div className="user-account-menu-name">
                      Hello,
                      <br />
                      {userData.user.BasicInformation.FullName.split(" ")[0]}!
                    </div>
                  </div>
                  {/* <Link  href={`https://unnanu.com/profilePro`} ><a className="dropdown-item unnanu-pro" href="https://unnanu.com/profilePro">Upgrade to PRO</a></Link> */}
                  <Link href={`${urls.Recruit}/more`}>
                    <a className="dropdown-item">Account</a>
                  </Link>
                  <Link href={`${urls.Recruit}/more/aboutme`}>
                    <a className="dropdown-item">About Me</a>
                  </Link>
                  <Link href={`${urls.Recruit}/more/referrals`}>
                    <a className="dropdown-item">Referrals</a>
                  </Link>
                  <Link href={`${urls.Recruit}/more/feedback`}>
                    <a className="dropdown-item">Feedback</a>
                  </Link>
                  <hr className="user-account-menu-hr" />
                  <Link href="#">
                    <a
